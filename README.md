...
| `cntv` | CCTV 官方 API，CCTV 全系列 + 30 个卫视，7 天覆盖 | 无限制 |
| `tvmao` | 电视猫 lighttv，覆盖大多数卫视/地方台，2 天覆盖 | 无限制（海外可访问，但有限速：~125次/IP） |
...
> 为缓解此问题，epg.pw 数据源每次抓取 **7 天**节目单覆盖，tvmao 抓取 **2 天**，即使某次定时任务延迟也不会影响当天节目单显示。
>
...
| `config/sources.yaml` | 抓取设置（各源独立天数、间隔、重试、限速熔断）|
