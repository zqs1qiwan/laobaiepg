...
- **多源聚合** — 支持 epg.pw、北京台官方(brtv)、电视猫(tvmao)、澳门 TDM 等多个数据源，自动换源
...
| `brtv` | 北京广播电视台官方 API（BTV2/3/4/5/7/9/10/12，数据精准）|
| `tvmao` | 电视猫（备用，部分地方台）|
...
>
> **数据保护机制**：每次抓取完成后，按日期维度与上次结果合并——新抓到的日期用新数据，未抓到的日期保留旧数据。某个频道某天临时抓取失败，不会清空该天已有的节目单。
